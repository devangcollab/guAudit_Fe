import { Box, Button, Stack, TextField, useMediaQuery } from "@mui/material";
import React, { useEffect, useState } from "react";
import { useTheme } from "@mui/material/styles";
import LiveLocationTable from "../tables/Location/LiveLocationTable";
import DeletedLocationTable from "../tables/Location/DeletedLocationTable";
import LocationDrawer from "../components/Drawer/LocationDrawer";

const LocationPage = () => {
  const [showLive, setShowLive] = useState(true);
  const [open, setOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [refreshkey,setRefreshkey] = useState(0)

  const toggleDrawer = (state) => {
    setOpen(state);
  };

  const theme = useTheme();
  const isSmallScreen = useMediaQuery(theme.breakpoints.down("sm")); // sm = <600px

  const refresh = () => {

    setRefreshkey((prev) => prev + 1)

  }
 

  return (
    <>
      <Stack
        spacing={2}
        direction={isSmallScreen ? "column" : "row"}
        justifyContent="space-between"
        alignItems={isSmallScreen ? "stretch" : "center"}
        flexWrap="wrap"
      >
        <Box display="flex" gap={2} flexWrap="wrap">
          <Button
            variant="contained"
            sx={{
              borderRadius: "3rem",
              px: 5,
              color: showLive ? "white" : "black",
              backgroundColor: showLive ? "#E8194F" : "white",
              whiteSpace: "nowrap",
            }}
            onClick={() => setShowLive(true)}
          >
            Location
          </Button>
          <Button
            variant="contained"
            sx={{
              borderRadius: "3rem",
              px: 5,
              color: showLive ? "black" : "white",
              backgroundColor: showLive ? "white" : "#E8194F",
              whiteSpace: "nowrap",
            }}
            onClick={() => setShowLive(false)}
          >
            Deleted / Archived
          </Button>
        </Box>

        {showLive  && (
          <Box mt={isSmallScreen ? 1 : 0}>
            <Button
              variant="contained"
              fullWidth={isSmallScreen}
              sx={{ borderRadius: "3rem", px: 5 }}
              onClick={() => setOpen(true)}
            >
              Add
            </Button>
          </Box>
        )}
      </Stack>
      {showLive && (
        <Stack
          mt={4}
          direction={isSmallScreen ? "column" : "row"}
          alignItems="center"
          spacing={2}
        >
          <TextField
            size="small"
            fullWidth
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search Location..."
          />
        </Stack>
      )}

      <Box mt={3}>
        {showLive ? (
          <LiveLocationTable search={searchTerm} refreshkey={refreshkey} />
        ) : (
          <DeletedLocationTable />
        )}
      </Box>

      <LocationDrawer open={open} toggleDrawer={toggleDrawer} refreshPage={refresh} />
    </>
  );
};

export default LocationPage;
